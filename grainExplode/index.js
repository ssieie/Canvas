...
class GrainExplode {
    ...
    constructor(id) {
        ...
        const mousemoveHandler = throttle(this.canvasMousemoveHandler, 50, { leading: true, trailing: false })
    ...
    directionLines() {
        ...
        for (let i = 0, len = this.lines.length; i < len; i++) {
            ...
            const nP = this.findEndpoint(l.targetX, l.targetY, l.currentX, l.currentY, l.cD)
            ...
            if (long > l.long) {
                const eP = this.findEndpoint(l.targetX, l.targetY, l.currentX, l.currentY, l.cD + l.long + 1)
                l.endX = eP.x
                l.endY = eP.y
            }
    ...
    createLineColor(line) {
        const gradient = this.$.createLinearGradient(line.endX, line.endY, line.nextX, line.nextY);
        gradient.addColorStop(0, `rgba(${line.color.r},${line.color.g},${line.color.b},0)`);
        gradient.addColorStop(0.5, `rgba(${line.color.r},${line.color.g},${line.color.b},0.3)`);
        gradient.addColorStop(1, `rgba(${line.color.r},${line.color.g},${line.color.b},1)`);
    ...
    createMoveLine(point) {
        ...
        const d = this.getTwoPointSpacing(this.bCenterPos.x, this.bCenterPos.y, point.x, point.y)
        this.lines.push({
            pId: point.id,
            destroy: false,
            targetX: point.x,
    ...
    createExplode(x, y) {
        ...
        for (let i = 0; i < num; i++) {
            ...
            for (let t = 0; t <= 1; t += 0.01) {
                const point = this.getCubicBezierPoint(x, y, cP1x, cP1y, cP2x, cP2y, endX, endY, t);
                e.path.push(point)
            }
    ...
    drawExplode() {
        for (let i = 0, len = this.explode.length; i < len; i++) {
            ...
            if (ppLen) {
                this.$.fillStyle = `rgba(${p.color.r},${p.color.g},${p.color.b},${ppLen / 100})`
                this.$.beginPath();
